...
def mostrar_progreso(palabra_secreta, letras_adivinandas):
    adivinado = ''
    ...
    while not juego_terminado and intentos > 0:
        ...
        if len(adivinanza) != 1 or not adivinanza.isalpha():
            ...
            if adivinanza in palabra_secreta:
                print(f"¡Muy bien has acertado, la letra '{adivinanza}' está presenta en la palbra ")
            else:
                intentos -= 1
                print(f"Lo siento mucho la letra '{adivinanza}' no está presente en la palabra secreta")
                print(f"Te quedan {intentos} itentos")
        ...
        if "_" not in progreso_actual:
            ...
            print(f"¡Felicitaciones has ganado! La palaba completa es: '{palabra_secreta}'")
            
    if intentos == 0:
        palabra_secreta = palabra_secreta.capitalize()
        print(f"Lo sentimos mucho se te han acabado los intentos, la palbra secreta era '{palabra_secreta}'")
